...
bool MyMesh::isChanBlacklisted(const mesh::Packet* packet) const {
  ...
  // Bytes after PATH_HASH_SIZE are MAC/ciphertext, not part of a stable
  // channel identifier, so comparisons must not extend past it.
  for (int b = 0; b < MAX_BLACKLIST_ENTRIES; b++) {
    if (_chan_blacklist[b].len == 0) continue;
    uint8_t avail   = (packet->payload_len < PATH_HASH_SIZE) ? (uint8_t)packet->payload_len : PATH_HASH_SIZE;
...
void MyMesh::deriveChanNameFilter(ChanNameFilter& entry, const char* name) {
  ...
  memset(entry.secret, 0, sizeof(entry.secret));  // zero-pad up to PUB_KEY_SIZE (MACThenDecrypt() reads the full width)
...
bool MyMesh::addChanNameFilter(const char* name) {
  // Reject names that deriveChanNameFilter() would silently truncate --
  // otherwise a truncated entry can be added but never matched/removed by
  // its original (longer) name.
  if (strlen(name) > sizeof(_chan_name_filters[0].name) - 1) return false;
...
void MyMesh::formatChanBlacklist(char* reply, size_t max_len) {
  char* dp = reply;
  char* end = reply + max_len - 1;  // leave room for the null terminator
  int count = 0;
...
void MyMesh::onPeerDataRecv(mesh::Packet *packet, uint8_t type, int sender_idx, const uint8_t *secret,
  ...
  if (type == PAYLOAD_TYPE_REQ) { // request (from a Known admin client!)
    ...
    if (!(flags == TXT_TYPE_PLAIN || flags == TXT_TYPE_CLI_DATA)) {
      ...
      uint8_t temp[CLI_REPLY_BUF_SIZE];
...
void MyMesh::handleCommand(uint32_t sender_timestamp, char *command, char *reply) {
  ...
  if (memcmp(command, "setperm ", 8) == 0) {   // format:  setperm {pubkey-hex} {permissions-int8}
    ...
    if (list && n >= 3 && strcmp(parts[2], "list") == 0) {
      if (is_chan) {
        formatChanBlacklist(reply, CLI_REPLY_MAX_LEN);
      } else {
      ...
      // Size the buffer for the larger of the two possible inputs: hex prefixes,
      // or up to MAX_CHAN_NAME_FILTERS '#name' entries (each up to 31 chars + '#').
      constexpr size_t MAX_HEX_TOKENS_LEN =
          MAX_PATH_PREFIX_LEN * 2 * MAX_BLACKLIST_ENTRIES + MAX_BLACKLIST_ENTRIES + 2;
      constexpr size_t MAX_NAME_TOKENS_LEN =
          32 * MAX_CHAN_NAME_FILTERS + MAX_CHAN_NAME_FILTERS + 2;
      constexpr size_t TOKENS_BUF_LEN =
          MAX_HEX_TOKENS_LEN > MAX_NAME_TOKENS_LEN ? MAX_HEX_TOKENS_LEN : MAX_NAME_TOKENS_LEN;
      char tokens[TOKENS_BUF_LEN];

      if (strlen(parts[3]) >= sizeof(tokens)) {
        strcpy(reply, "Err - input too long");
        return;
      }
      ...
      for (int t = 0; t < tok_n; t++) {
        if (is_chan && tok_parts[t][0] == '#') {
          ...
          // For channel entries, only PATH_HASH_SIZE bytes of the hex prefix are
          // ever actually compared against (see isChanBlacklisted()), so reject
          // longer entries up front rather than silently accepting a prefix that
          // can never match anything beyond its first PATH_HASH_SIZE bytes.
